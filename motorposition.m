...
%Response to a Step Reference with Different Values of K_p
Kp = 1;
for i = 1:3
    C(:,:,i) = pid(Kp);
    Kp = Kp + 10;
...
%Response to a Step Disturbance with Different Values of K_p
...
legend('Kp = 1', 'Kp = 11','Kp = 21')
%Response to a Step Reference with K_p = 21 and Different Values of K_i
Kp = 21;
Ki = 100;
for i = 1:5
    C(:,:,i) = pid(Kp,Ki);
...
%Response to a Step Disturbance with K_p = 21 and Different Values of K_i
dist_cl = feedback(P_motor,C);
figure(4)
step(dist_cl(:,:,1), dist_cl(:,:,2), dist_cl(:,:,3), t)
ylabel('Position, \theta (radians)')
